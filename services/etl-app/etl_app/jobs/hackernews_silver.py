...
def main(date: str) -> None:
    ...
    if df.is_empty():
        print(f"[hackernews_silver] no bronze rows for {date}")
        return

    # Keep latest ingest per (story_id, created_at); then drop ingest_ts.
    df = (
        df.sort("ingest_ts", descending=True)
        .unique(subset=["story_id", "created_at"], keep="first")
        .drop("ingest_ts")
    ...
    result = silver.upsert(arrow, join_cols=["story_id", "created_at"])
    silver.refresh()
    print(
        f"[hackernews_silver] {date}: inserted={result.rows_inserted} "
