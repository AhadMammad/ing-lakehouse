"""Daily global air quality ETL: OpenAQ → bronze → silver → gold Iceberg tables.
...
Prerequisites:
  OPENAQ_API_KEY     — free API key from openaq.org/register
  OPENAQ_LOCATION_IDS — comma-separated OpenAQ location IDs to ingest
                        (find IDs at explore.openaq.org)
"""
...
COMMON = dict(
    ...
    environment={
        ...
        "OPENAQ_LOCATION_IDS": os.environ.get("OPENAQ_LOCATION_IDS", ""),
    },
)

with DAG(
    dag_id="airquality_etl_global",
    schedule="0 8 * * *",  # daily 08:00 UTC — after earthquake ETL (07:00)
    start_date=datetime(2026, 5, 1),
    catchup=False,
    max_active_runs=1,
    tags=["airquality", "openaq", "lakehouse", "medallion"],
    description="OpenAQ global hourly air quality (PM2.5/PM10/NO2/O3) → bronze/silver/gold Iceberg tables",
    ...
    bronze = DockerOperator(
        task_id="bronze",
        command="etl_app.jobs.airquality_bronze --date {{ ds }}",
        **COMMON,
    )
    silver = DockerOperator(
        task_id="silver",
        command="etl_app.jobs.airquality_silver --date {{ ds }}",
        **COMMON,
    )
    gold = DockerOperator(
        task_id="gold",
        command="etl_app.jobs.airquality_gold --date {{ ds }}",
        **COMMON,
    )
