...
with DAG(
    dag_id="hackernews_etl",
    schedule="0 6 * * *",  
    start_date=datetime(2026, 5, 1),
    catchup=False,
    max_active_runs=1,
    tags=["hackernews", "lakehouse", "medallion"],
    description="HackerNews → bronze/silver/gold Iceberg tables",
    ...
    bronze = DockerOperator(
        task_id="bronze",
        command="etl_app.jobs.hackernews_bronze --date {{ ds }}",
        **COMMON,
    )
    silver = DockerOperator(
        task_id="silver",
        command="etl_app.jobs.hackernews_silver --date {{ ds }}",
        **COMMON,
    )
    gold = DockerOperator(
        task_id="gold",
        command="etl_app.jobs.hackernews_gold --date {{ ds }}",
        **COMMON,
    )
